...
export const registerMutation = {
  async userRegistration(__: unknown,{ data }: { data: IReg }): Promise<registerResponse> {
    ...
    try {
      ...
      const verificationLink = `https://arc-creatives.fly.dev/?action=verifyEmail&token=${token}`
  ...
  async forgotPassword(_: any, { data }: { data: { email: string } }): Promise<Generalres> {
    try {
      ...
      if (userExist) {
        ...
        let link = `https://arc-creatives.fly.dev/?action=resetpassword&email=${email}&token=${token}`; // for dev 
